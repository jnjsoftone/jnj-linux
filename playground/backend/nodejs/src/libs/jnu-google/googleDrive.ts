 ...
 *   [X] createSpreadsheet - Google Sheets 생성
 *   [X] createDocument - Google Docs 생성
 *   [X] searchFiles - 파일 검색
 *   [X] copyFile - 파일 복사
 *   [X] moveFile - 파일 이동
...
export class GoogleDrive {
  ...
  /**
   * Static factory method to create and initialize GoogleDrive instance
   * @example
   * const drive = await GoogleDrive.create({ user: 'ilinkrun' });
   * const files = await drive.listFiles();
   */
  static async create(options?: { user?: string; type?: string; sn?: number; scopeDir?: string; authDir?: string }): Promise<GoogleDrive> {
    const instance = new GoogleDrive(options || {});
    await instance.init();
    return instance;
  }
   ...
   * @param spreadsheetName - 스프레드시트 이름
   * @param parentId - 상위 폴더 ID (없으면 루트 폴더에 생성)
   * @returns 생성된 스프레드시트 정보
   * @example
   * const drive = await GoogleDrive.create({ user: 'ilinkrun' });
   * const sheet = await drive.createSpreadsheet('My Sheet');
   * console.log(sheet.id, sheet.webViewLink);
   */
  createSpreadsheet = async (spreadsheetName: string, parentId?: string) => {
    const fileMetadata: DriveFile = {
   ...
   * @example
   * const drive = await GoogleDrive.create({ user: 'ilinkrun' });
   * const doc = await drive.createDocument('My Document');
   * console.log(doc.id, doc.webViewLink);
   */
  createDocument = async (documentName: string, parentId?: string) => {
    const fileMetadata: DriveFile = {
      name: documentName,
