...
export class GoogleGmail {
  ...
  /**
   * Static factory method to create and initialize GoogleGmail instance
   * @example
   * const gmail = await GoogleGmail.create({ user: 'ilinkrun' });
   * const messages = await gmail.listGmails();
   */
  static async create(options?: { user?: string; type?: string; sn?: number; scopeDir?: string; authDir?: string }): Promise<GoogleGmail> {
    const instance = new GoogleGmail(options || {});
    await instance.init();
    return instance;
  }
