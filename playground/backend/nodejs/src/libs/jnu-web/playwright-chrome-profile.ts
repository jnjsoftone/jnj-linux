...
const getPlaywrightChromeProfileByEmail = (email = '', userDataDir = '') => {
  // userDataDir가 비어있으면 CHROME_USERDATA_PATH 사용
  if (!userDataDir) {
    userDataDir = process.env.CHROME_USERDATA_PATH || '/root/.config/google-chrome';
  }
...
class PlaywrightChromeProfile {
  ...
  private async initializeBrowser(options: {
    ...
    // 프로필 경로 설정
    const baseUserDataDir = options.userDataDir || process.env.CHROME_USERDATA_PATH || '/Users/youchan/Library/Application Support/Google/Chrome';
